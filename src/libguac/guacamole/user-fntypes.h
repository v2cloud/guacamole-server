 ...
 *    Identifies which monitor slot is being addressed; monitor 0 is the
 *    primary.
 *
 * @param top_offset
 ...
 *   back to its legacy layout convention (the RDP handler treats it
 *   as a request to use the horizontal-row cumulative-sum-by-
 *   x_position computation, matching pre-non-linear-layout clients).
 *   The libguac wire-handler maps an absent 5th `size` opcode
 *   argument to INT_MIN; protocol implementations that consume this
 *   handler should test for INT_MIN explicitly rather than treating
 *   it as a regular coordinate.
 *
 * @return
